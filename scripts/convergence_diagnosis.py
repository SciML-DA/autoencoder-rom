...
from tools.epod import pod as _pod  # noqa: E402
...
def closed_form(Q, Sd, tr, te, args, n_sub=None, label=""):
    ...
    t0 = time.time()
    m = PODLSE(r_field=args.r_field, r_sensor=args.r_sensor, ridge=args.ridge,
               pod_method=args.pod_method).fit(Q[:, idx], Sd[:, idx])
    pred = m.predict(Sd[:, te])
    out = dict(label=label, n_fit=len(idx), nmse_test=nmse(Q[:, te], pred),
               nmse_train=m.score(Q[:, idx], Sd[:, idx]),
               seconds=time.time() - t0)
    print(f"  {label:<42} n_fit {len(idx):>5}   train {out['nmse_train']:.4f}"
...
def fit_branch(Q, S, tr, te, lat, args, *, branch, epochs, lr, val_fraction,
               seed=0, label=""):
    """One branched fit, returning scores and what the training actually did."""
    t0 = time.time()
    m = BranchedAE(lat, branch=branch, n_delays=args.n_delays,
                   delay_stride=args.delay_stride, hidden=tuple(args.hidden),
                   gru_hidden=args.gru_hidden,
                   cnn_channels=tuple(args.cnn_channels),
                   latent_weight=args.latent_weight, learning_rate=lr,
                   n_epochs=epochs, batch_size=args.batch,
                   val_fraction=val_fraction, patience=args.patience,
                   seed=seed, device=args.device).fit(Q, S, tr)
    pred = m.predict(S, te)
    hist = list(m.loss_history or [])
    out = dict(label=label, branch=branch, epochs=epochs, lr=lr,
               val_fraction=val_fraction, seed=seed,
               nmse_test=nmse(Q[:, te], pred),
               nmse_train=m.score(Q, S, tr),
               cos_test=cosine(Q[:, te] - Q[:, te].mean(1, keepdims=True),
                               pred - pred.mean(1, keepdims=True)),
               energy_test=energy_ratio(Q[:, te] - Q[:, te].mean(1, keepdims=True),
                                        pred - pred.mean(1, keepdims=True)),
               epochs_run=len(hist),
...
def section_epochs(Q, S, tr, te, lat, args, ref_full, ref_sub):
    ...
    rows, curves = [], {}
    print(f"  {'epochs':>7} {'val_frac':>9} {'run':>6} {'stop':>5} "
          f"{'train':>8} {'test':>8} {'gap to CF':>10} {'s':>6}")
    for vf in args.val_fractions:
        for ep in args.epoch_grid:
            r, hist = fit_branch(Q, S, tr, te, lat, args, branch="linear",
                                 epochs=ep, lr=args.lr, val_fraction=vf,
            ...
            rows.append(r)
            curves[(vf, ep)] = hist
            print(f"  {ep:>7} {vf:>9.2f} {r['epochs_run']:>6} "
...
def _verdict_epochs(rows, ref_full, ref_sub):
    ...
    if vf0:
        ...
        print(f"    branched {b0['nmse_test']:.4f}  vs  closed form "
              f"{ref_full['nmse_test']:.4f}   gap {d:+.4f}")
...
def section_lr(Q, S, tr, te, lat, args, ref_full):
    ...
    for lr in args.lr_grid:
        r, _ = fit_branch(Q, S, tr, te, lat, args, branch="linear",
                          epochs=max(args.epoch_grid), lr=lr, val_fraction=0.0,
                          label=f"linear lr{lr}")
        r["gap"] = r["nmse_test"] - ref_full["nmse_test"]
        r["section"] = "lr"
        rows.append(r)
        print(f"  {lr:>9.1e} {r['epochs_run']:>6} {r['nmse_train']:>8.4f} "
              f"{r['nmse_test']:>8.4f} {r['gap']:>+10.4f}")
    b = min(rows, key=lambda r: r["nmse_test"])
    print(f"\n  best lr {b['lr']:.1e} -> {b['nmse_test']:.4f} "
          f"(gap {b['gap']:+.4f})")
...
def section_performance(Q, S, tr, te, latents, args, ref_full):
    ...
    rows = []
    print(f"  {'model':<16}{'train':>8}{'test':>8}{'cos':>7}{'E':>6}"
          f"{'epochs':>8}{'stop':>6}{'params':>11}{'s':>6}")
    print(f"  {'podlse (CF)':<16}{ref_full['nmse_train']:>8.4f}"
          f"{ref_full['nmse_test']:>8.4f}{'-':>7}{'-':>6}{'-':>8}{'-':>6}"
          f"{'-':>11}{ref_full['seconds']:>6.0f}")
    for lk, lat in latents.items():
        for br in args.branches:
            r, _ = fit_branch(Q, S, tr, te, lat, args, branch=br,
                              epochs=args.perf_epochs, lr=args.lr,
                              val_fraction=args.perf_val_fraction,
                              label=f"{lk}+{br}")
            r["section"] = "performance"
            rows.append(r)
            print(f"  {lk + '+' + br:<16}{r['nmse_train']:>8.4f}"
    ...
    if rows:
        ...
        print(f"\n  best network {b['label']} {b['nmse_test']:.4f} vs closed form "
              f"{ref_full['nmse_test']:.4f}  ({d:+.4f})")
        print("  " + ("a network wins" if d < -0.005 else
                      "the closed-form linear estimator is still the best model"))
    return rows
...
def section_learning_curve(Q, S, tr, te, args):
    ...
    print(f"  {'L':>5} {'n_par':>9} {'n_train':>8} {'par/snap':>9} "
          f"{'train':>8} {'test':>8}")
    for L in args.lc_delays:
        ...
        for frac in args.train_fractions:
            ...
            m = PODLSE(r_field=args.r_field, r_sensor=r_s, ridge=args.ridge,
                       pod_method=args.pod_method).fit(Q[:, idx], Sd[:, idx])
            r = dict(section="learning_curve", n_delays=L, frac=frac,
                     n_train=len(idx), n_params=m.n_params,
                     nmse_train=m.score(Q[:, idx], Sd[:, idx]),
                     nmse_test=nmse(Q[:, te], m.predict(Sd[:, te])))
            rows.append(r)
            print(f"  {L:>5} {m.n_params:>9,d} {len(idx):>8} "
...
def _verdict_learning_curve(rows, args):
    ...
    verdicts = []
    for L in args.lc_delays:
        rs = sorted([r for r in rows if r["n_delays"] == L],
        ...
        print(f"    L={L:<4} {a['n_train']:>5} -> {b['n_train']:>5} snapshots: "
    ...
    if slope > 0.01:
        ...
        print(f"  VERDICT: still falling at the full training block "
              f"({slope:+.4f} per doubling at L={L}).")
...
def make_plots(ep_rows, curves, lr_rows, perf_rows, lc_rows, ref_full,
               ref_sub, out):
    fig, ax = plt.subplots(1, 4, figsize=(21, 4.6))

    a = ax[0]
    for vf in sorted({r["val_fraction"] for r in ep_rows}):
        rs = sorted([r for r in ep_rows if r["val_fraction"] == vf],
                    key=lambda r: r["epochs"])
        a.semilogx([r["epochs"] for r in rs], [r["nmse_test"] for r in rs],
                   "o-", label=f"pod+linear, val_fraction {vf:g}")
    a.axhline(ref_full["nmse_test"], c="k", ls="--", lw=1.4,
              label="closed form, full block")
    a.axhline(ref_sub["nmse_test"], c="C3", ls=":", lw=1.4,
              label="closed form, same subset")
    a.set(xlabel="training epochs", ylabel="test NMSE",
          title="optimisation convergence against a known optimum")
    a.legend(fontsize=7)
    ...
    for (vf, ep), h in sorted(curves.items()):
        if h and ep == max(r["epochs"] for r in ep_rows):
            a.loglog(np.arange(1, len(h) + 1), h, lw=1.2,
                     label=f"val_fraction {vf:g}")
    a.set(xlabel="epoch", ylabel="training loss",
          title="training loss -- is it still descending?")
    a.legend(fontsize=7)
    a.grid(alpha=0.3, which="both")
    ...
    if lc_rows:
        for L in sorted({r["n_delays"] for r in lc_rows}):
            rs = sorted([r for r in lc_rows if r["n_delays"] == L],
                        key=lambda r: r["n_train"])
            a.semilogx([r["n_train"] for r in rs], [r["nmse_test"] for r in rs],
                       "o-", label=f"L={L}")
        a.axhline(ref_full["nmse_test"], c="k", ls="--", lw=1.2,
                  label="closed form, full block")
        a.set(xlabel="training snapshots", ylabel="test NMSE",
              title="learning curve -- is one run enough data?")
        a.legend(fontsize=7)
        a.grid(alpha=0.3, which="both")
    ...
    if perf_rows:
        ...
        a.barh(y, [r["nmse_train"] for r in rs], color="C1", height=0.45,
               label="train")
        a.axvline(ref_full["nmse_test"], c="k", ls="--", lw=1.4,
                  label="closed form")
        a.set_yticks(y)
...
def main() -> int:
    ...
    d.add_argument("--train-fractions", type=float, nargs="+",
                   ...
                   help="learning curve: fractions of the training block, taken "
                        ...
                        "held constant")
    d.add_argument("--lc-delays", type=int, nargs="+", default=[10, 25, 50, 100],
                   help="learning curve: window lengths, since the parameter "
                        "count scales with L")
    d.add_argument("--skip", nargs="*", default=[],
                   choices=["epochs", "lr", "performance", "learning_curve"])
    ...
    o.add_argument("--quick", action="store_true",
                   help="tiny grids, for checking the script runs at all")
    args = p.parse_args()
    ...
    ref_full = closed_form(Q, Sd, tr, te, args,
                           label="podlse, full training block")
    n_sub = int(round(len(tr) * (1.0 - max(args.val_fractions))))
    ref_sub = closed_form(Q, Sd, tr, te, args, n_sub=n_sub,
                          label=f"podlse, first {n_sub} only (branch's budget)")
    print(f"\n  the holdout alone is worth "
          f"{ref_sub['nmse_test'] - ref_full['nmse_test']:+.4f} NMSE")
    ...
    if "pod" in args.latents:
        Psi, _, _, qm = _pod(Q[:, tr], r=args.r_field, subtract_mean=True,
    ...
    if "ae" in args.latents:
        ...
        p_ae = AE(n_latent=args.r_field, layer_dims=dims,
                  n_epochs=args.ae_epochs, batch_size=args.ae_batch,
                  learning_rate=args.ae_lr, patience=args.ae_patience,
                  device=args.device, seed=0).fit(grid)
        latents["ae"] = TorchLatent(p_ae, device=args.device)
        print(f"  AE latent r={args.r_field} dims={dims}: {time.time() - t0:.1f}s, "
              f"{p_ae.n_params / 1e6:.2f}M params")

    rows, curves = [], {}
    if "epochs" not in args.skip:
        r, curves = section_epochs(Q, S, tr, te, latents["pod"], args,
                                   ref_full, ref_sub)
        rows += r
    lr_rows = []
    ...
    make_plots([r for r in rows if r.get("section") == "epochs"], curves,
               lr_rows, perf_rows, lc_rows, ref_full, ref_sub, out)
