...
def snapshot_pod_randomized(Q, n_modes=20, n_iter=4, random_state=None):
    ...
    try:
        ...
        U, s, _ = randomized_svd(Q, n_components=n_modes, n_iter=n_iter, random_state=random_state)
    except ImportError:
...
def spod_towne(Q, dt=1.0, n_fft=None, n_ovlp=None, window="hamming", weight=None, conf_level=0.95):
    """
    ...
    if n_blks < 2:
        raise ValueError(f"Too few blocks ({n_blks}). Reduce n_fft/n_ovlp or use more snapshots.")
    ...
    Lc = np.stack([L * 2 * n_blks / xi2_lo, L * 2 * n_blks / xi2_up], axis=-1)
    info = dict(n_fft=n_fft, n_ovlp=n_ovlp, n_blks=n_blks, window=win, n_freq=n_freq)
...
def spod_towne_reconstruct(Psi, A_blk, n_fft, n_ovlp, N_t):  # noqa: ARG001
    ...
    raise NotImplementedError(
        "Full reconstruction (inverse SPOD) is not yet implemented. See Nekkanti & Schmidt (JFM 2021) for details."
    )
