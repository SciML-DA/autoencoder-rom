...
class EchoStateNetwork:
    ...
    input_parameters: Optional[np.ndarray] = (
        None  # TODO: add input parameters functionality to enable parametric ESNs
    )
    ...
    def __init__(self, y, dt=1, **kwargs):
        ...
        if y.ndim == 1:
            ...
            raise AssertionError(
                f"y.shape={y.shape}. The input y must have 2 dimension"
            )

        #   Initialise state dimensions and reservoir state to zeros ------------ #
        self.N_dim = y.shape[
            0
        ]  # Dimension of the physical system i.e., the output dimension
        self.observed_idx = kwargs.pop(
            "observed_idx", np.arange(self.N_dim)
        )  # Default to full observability
    ...
    def WCout(self, value=None):
        ...
        assert (
            self.trained
        ), "ESN must be trained with washout before calling step method. Call ESN.train() first."
        if value is None:
            self._WCout = np.linalg.lstsq(self.Wout[:-1], self.W.toarray(), rcond=None)[
                0
            ]
        else:
            self._WCout = value
    ...
    def sparsity(self):
        ...
        """
        return 1.0 - self.connect / (self.N_units - 1)
    ...
    def step(self, u, r):
        ...
        if u.ndim == 1:
            ...
            assert (
                u.shape[0] == 1
            ), f"Input u has shape {u.shape}, only 1 sample at a time is allowed"
            u = u[0]
        if r.ndim == 1:
            ...
            assert (
                r.shape[0] == 1
            ), f"Input r has shape {r.shape}, only 1 sample at a time is allowed"
            r = r[0]
    ...
    def outputs_to_inputs(self, full_state):
        ...
        """
        assert (
            full_state.shape[0] == self.N_dim
        ), f"full_state has shape {full_state.shape}, expected first dim to be {self.N_dim}"

        observed_state = full_state[self.observed_idx]

        assert (
            observed_state.shape[0] == self.N_dim_in
        ), f"observed_state has shape {observed_state.shape}, expected first dim to be {self.N_dim_in}"
    ...
    def Jacobian(self, u_in, r_in, open_loop_J=True):
        ...
        assert (
            self.trained
        ), "ESN must be trained before computing the Jacobian. Call ESN.train() first."
        ...
        if not open_loop_J:
            ...
            raise NotImplementedError(
                "Numerical test of closed-loop Jacobian did not pass"
            )
    ...
    def train(
        ...
        **kwargs,
        ...
        U_wtv, Y_wtv, U_test, Y_test = self._split_and_format_data(
        ...
        self.Wout = np.zeros(
            (self.N_units + 1, self.N_dim)
        )  # Initialize Wout with zeros
        ...
        if self.hyperparameters_to_optimize:
            bo_results = self._optimize_hyperparameters(
                U_wtv, Y_wtv, validation_strategy, print_convergence=plot_training
            )
        else:
            bo_results = None
        ...
        if plot_training:
            self._plot_training_results(
                U_test, Y_test, bo_results, save_ESN_training, folder
            )
    ...
    def _generate_W_Win(self, seed=None):
        ...
        if not hasattr(self, "_Win"):
            Win = lil_matrix(
                (self.N_units, self.N_dim_in + 1)
            )  # +1 accounts for input bias
            if self.Win_type == "sparse":
                for j in range(self.N_units):
                    Win[j, rng0.choice(self.N_dim_in + 1)] = rng0.uniform(
                        low=-1, high=1
                    )
                ...
                raise ValueError(
                    "Win type {} not implemented ['sparse', 'dense']".format(
                        self.Win_type
                    )
                )
            # Store
        ...
        if not hasattr(self, "_W"):
            W = csr_matrix(
                rng0.uniform(low=-1, high=1, size=(self.N_units, self.N_units))
            ...
            spectral_radius = np.abs(
                sparse_eigs(W, k=1, which="LM", return_eigenvectors=False)
            )[0]
            self.W = (1.0 / spectral_radius) * W
        ...
        for ll in range(U_wtv.shape[0]):
            ...
            for U_t, Y_t in zip(U_train, Y_target):
                if Y_t.ndim == 3:
                    assert (
                        Y_t.shape[-1] == 1
                    ), f"Y_t has shape {Y_t.shape}, only 1 sample at a time is allowed"
                    Y_t = Y_t[..., 0]
    ...
    def _split_and_format_data(self, data=None, add_noise=True):
        ...
        if data is None:
            raise ValueError(
                "No training data provided to format_training_data method."
            )
        if data.ndim == 2:
            data = np.expand_dims(data, axis=0)

        U, Y = self._UY_from_raw_data(
            data, add_noise=add_noise
        )  # dimensions: L x Nt x N_dim_in/N_dim
        ...
        if U.shape[1] < N_wtv:
            raise ValueError(
                f"Increase the length of the training data signal. {U.shape} < {N_wtv}"
            )
    ...
    def _optimize_hyperparameters(
        self, U_wtv, Y_wtv, validation_strategy=None, print_convergence=True
    ):
        """
        ...
        search_grid, search_space, hp_names = self._hyperparameter_search(
        ...
        kernel_ = ConstantKernel(
            constant_value=1.0, constant_value_bounds=(1e-1, 3e0)
        ) * Matern(
            length_scale=[0.2] * len(search_space),
            nu=2.5,
        ...
        return dict(res=result, hp_names=hp_names)
        ...
        if print_convergence:
            print(
                ...
                + " and {} points with Bayesian Optimization\n\t".format(
                    self.N_func_evals - self.N_grid**2
                ),
                end="",
            )
    ...
    def _set_norm(train_data, method=None):
        ...
        if train_data.ndim == 3:
            ...
            raise ValueError(
                f"U_wtv must be a 2D, 3D or 4D array, got {train_data.ndim}D: ({train_data.shape})"
            )
        ...
        for U_l, Y_l in zip(U_wtv, Y_wtv):  # Each set of training data
            ...
            for fold in range(case.N_folds):
                ...
                for tik_j in range(N_tikh):  # cloop for each tikh_-noise combination
                    ...
                    nRMSE[tik_j] += np.log10(
                        case.compute_nRMSE(Y_val, Y_closed, norm=norm_l)
                    )
    ...
    def run_test(
        ...
        if L > 1:
            if max_L_tests != L:
                L_indices = np.sort(
                    rng0.choice(L, max_L_tests, replace=max_L_tests > L)
                )
            else:
        ...
        # Plotting function
        def plot_time(
            _axs, _time, _pred_closed, _pred_open, _inputs, _target, _err=None
        ):
            ...
            for dim_i, _ax in zip(range(self.N_dim), _axs):
                ...
                _ax.plot(
                    ...
                    label=[
                        (
                            f"ESN closed-loop prediction \n error = {_err:.4}"
                            if _err is not None
                            else f"ESN closed-loop prediction"
                        )
                    ],
                )
        ...
        hist_args = dict(
            bins=nbins, density=True, orientation="horizontal", stacked=False
        )
        ...
        for Li in L_indices:
            ...
            ylims = [
                [np.min(Y_test_l[:, dim_i]) * 1.05, np.max(Y_test_l[:, dim_i]) * 1.05]
            ...
            if long_term:
                ...
                err_long = np.log10(
                    self.compute_nRMSE(Y_closed, Y_test_l[self.N_wash :], norm=norm_l)
                )

                fig_long, grid = plt.subplots(
                    nrows=self.N_dim,
                    ncols=2,
                    figsize=[10, 2.5 * self.N_dim],
                    sharex="col",
                    sharey="row",
                    layout="tight",
                    width_ratios=[5, 1],
                ...
                plt.suptitle(
                    f"Li = {Li}, observed idx = {self.observed_idx}, error = {err_long:.4}"
                )
            ...
            if short_term:
                ...
                while i0 + Nt_test < max_test_time:
                    ...
                    current_error = np.log10(
                        self.compute_nRMSE(current_target, Y_closed, norm=norm_l)
                    )

                    short_term_error += current_error

                    if test_counter <= max_L_tests:
                        fig_short, axs_short = plt.subplots(
                            nrows=nrows,
                            ncols=1,
                            figsize=[8, 1.5 * nrows],
                            sharex="all",
                        ...
                        plot_time(
                            ...
                            _pred_closed=Y_closed,
                            _pred_open=U_open,
                            _inputs=current_input,
                            _target=current_target,
                            _err=current_error,
                        )

                        axs_short[0].legend(
                            title=f"Test {test_counter}: Li = {Li}",
                            loc="upper left",
                            bbox_to_anchor=(1, 1),
                            fontsize="x-small",
                        )
        ...
        if test_counter > 0:
            ...
            print(
                f"Overall tests min, max and mean MSE in {test_counter} tests = {np.min(errors_all):.4}, {np.max(errors_all):.4}, {np.mean(errors_all):.4}."
            )

        return [fig_long] + figures_short

    def _plot_training_results(
        self, U_test, Y_test, results, save_ESN_training, folder
    ):
        """
        ...
        if self.perform_test and U_test.shape[1] >= self.N_val:
            test_figs = self.run_test(
                U_test, Y_test, long_term=True, short_term=True, max_short_tests=5
            )
    ...
    def _plot_BO(self, results_bayesian_optimization):
        ...
        if len(hp_names) >= 2:  # plot GP reconstruction
            ...
            for hpi in range(len(hp_names) - 1):
                range_1 = getattr(
                    self, f"{hp_names[hpi]}_range"
                )  # type: tuple[float, float]
                range_2 = getattr(
                    self, f"{hp_names[hpi + 1]}_range"
                )  # type: tuple[float, float]

                n_len = 100  # points to evaluate the GP at
                xx, yy = np.meshgrid(
                    np.linspace(*range_1, n_len), np.linspace(*range_2, n_len)
                )

                x_x = np.column_stack((xx.flatten(), yy.flatten()))
                x_gp = res.space.transform(
                    x_x.tolist()
                )  # gp prediction needs norm. format
                ...
                y_pred = np.clip(
                    -gp.predict(x_gp), a_min=-amin, a_max=-np.min(f_iters)
                ).reshape(n_len, n_len)
                ...
                for rx, mk in zip(
                    [res_x[: self.N_grid**2], res_x[self.N_grid**2 :]], ["v", "s"]
                ):
                    plt.plot(rx[:, 0], rx[:, 1], mk, c="w", alpha=0.8, mec="k", ms=8)
                # Plot best point
                best_idx = np.argmin(f_iters)
                plt.plot(
                    res_x[best_idx, 0],
