...
and the `sensor_basis` ranking are both documented in `field_estimation/epod.py` as
...
from experiments.april_wake.data_preprocessing import add_data_args, band_limit, load_data  # noqa: E402
...
def main() -> int:
    ...
    if args.band_hz:
        ...
        band = float(args.band_hz[0]) if len(args.band_hz) == 1 else "-".join(f"{b:g}" for b in args.band_hz)
    else:
        band = None
    ...
    cfgs = []
    for nd, rf, rs, lam, sb in itertools.product(
        *(GRID[k] for k in ("n_delays", "r_field", "r_sensor", "ridge", "sensor_basis"))
    ):
        ...
        if rs is not None and rs >= bound:
            continue
        cfgs.append(
            dict(band_hz=band, n_delays=nd, r_field=rf, r_sensor=rs, ridge=lam, sensor_basis=sb, rank_bound=bound)
        )
    ...
    rule(f"band {band if band else 'fullband'}: {len(cfgs)} configs, {len(todo)} to run")
    ...
    for i, c in enumerate(todo):
        ...
        row["nmse_test"] = nmse(Q[:, te], m.predict(Sd[:, te])) if args.with_test else float("nan")
        ...
        print(
            f"  [{i + 1:>4}/{len(todo)}] L={nd:<4} r_f={c['r_field']:<4} "
