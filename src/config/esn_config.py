...
class ESNConfig:
    ...
    def save(self, save_dir: Path):
        ...
        print(
            f"Configuration saved to ...{'/'.join(list(save_dir.parts)[-3:])}/esn_config.yaml"
        )
    ...
    def load(cls, load_dir: Path, verbose=1):
        ...
        try:
            ...
            if verbose:
                print(
                    f"Configuration loaded from ...{'/'.join(list(load_dir.parts)[-3:])}/esn_config.yaml"
                )
            return cls(**config_dict)
    ...
    def save_trained_matrices(esn_model, save_dir: Path):
        ...
        print(
            f"Trained matrices saved to ...{'/'.join(list(save_dir.parts)[-3:])}/trained_matrices.npz"
        )
...
def save_esn_model_to_config(
    esn_model,
    save_dir: Union[str, Path] = Path(BASE_CONFIG_DIR) / "esn_configs",
...
def load_esn_model_from_config(
    q: Optional[str] = None,
    ...
    if q is not None:
        ...
        raise ValueError(
            "Either query string q or config instance must be provided to load ESN_model"
        )
...
def find_matching_config(
    search_dir: Union[str, Path], query_hash: str
) -> Optional[Path]:
    """
...
def auto_load_or_create(
    config_dir: Union[str, Path] = Path(BASE_CONFIG_DIR) / "esn_configs",
    auto_save: bool = True,
