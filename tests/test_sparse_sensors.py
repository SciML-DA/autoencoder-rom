"""Checks for the sparse-sensor reconstruction stack.

Runs against a synthetic fixture, so no experimental data or network access is
needed. The end-to-end checks shell out to the driver scripts and are gated
behind VERIFY_SLOW=1.

    pytest tests/                      # unit checks only
    VERIFY_SLOW=1 pytest tests/ -q     # + the study, sweep and video drivers
    pytest tests/ -k epod -v           # a subset, with per-check numbers

The fixture is an exactly rank-r field with a known linear sensor map, so the
leading checks assert exact recovery rather than a tolerance: POD-LSE is the
exact model there, and anything above 1e-10 is a bug in the linear algebra.
"""
...
# minutes; opt in with VERIFY_SLOW=1.
_SLOW = os.environ.get("VERIFY_SLOW") == "1"


@pytest.fixture
def v(pytestconfig) -> bool:
    """True under `pytest -v`, which turns on the per-check detail prints."""
...
def test_pod_recovers_known_rank(v):
    ...
    assert n == c["rank"], f"{n} modes above threshold, expected {c['rank']}"


def test_pod_orthonormal_and_exact(v):
    ...
    assert ok, f"orth {orth:.1e}, recon {recon:.1e}, cycle {cycle:.1e}"


def test_pod_methods_agree(v):
    ...
    assert ok, f"sigma {d_sig:.1e}, subspace {d_sub:.1e}"


# ── 2. the linear estimators ──────────────────────────────────────────────────


def test_lse_exact_recovery(v):
    ...
    e = m.score(c["Q"], c["S"])
    assert e < 1e-10, f"nmse {e:.2e} (need < 1e-10)"


def test_lse_map_solves_least_squares(v):
    ...
    assert ok, f"vs lstsq {d:.1e}, residual orthogonality {orth:.1e}"
...
def test_epod_is_borees_formula(v):
...
def test_extended_pod_recovers_sensor_map(v):
    ...
    assert d < 1e-10, f"rel err {d:.2e}"


def test_ridge_limits_are_sane(v):
    ...
    assert ok, f"small {small:.1e}, large {big:.1e}"


def test_prediction_equals_encode_then_expand(v):
    ...
    assert d < 1e-12, f"max |diff| {d:.1e}"


def test_standardisation_handles_mixed_units(v):
    ...
    assert d < 1e-8, f"|d nmse| {d:.1e}"


def test_error_grows_with_sensor_noise(v):
    ...
    assert ok, " < ".join(f"{e:.2e}" for e in errs)


def test_linear_floor_under_nonlinearity(v):
    ...
    assert ok, f"linear {out['linear']:.1e}, quadratic {out['quadratic']:.1e}"


# ── 3. observability, floors and the rank ceiling ─────────────────────────────


def test_observability_is_one_for_linear_sensors(v):
    ...
    assert ok, f"full min {full.min():.6f}, partial sum {partial.sum():.2f}/{c['rank']}"
...
def test_projection_floor_bounds_every_estimator(v):
    ...
    ok = got >= floor * (1 - 1e-9)
    assert ok, f"floor {floor:.4f} <= score {got:.4f}"


def test_delay_embed_is_causal(v):
    ...
    assert bool(ok), "blocks, padding and identity"


def test_delay_embed_raises_rank(v):
    ...
    assert e1 < e0 / 10, f"{e0:.2e} -> {e1:.2e} with 21 lags"


def test_no_leakage(v):
    ...
    assert contig > rand, f"contiguous {contig:.3e} > random {rand:.3e}"


def test_blocked_folds_respect_the_gap(v):
    ...
    assert bool(ok), f"{len(seen)} folds, no overlap, gap honoured"


# ── 4. the two-branch autoencoder ─────────────────────────────────────────────


def test_sensor_windows_match_delay_embed(v):
    ...
    assert ok, f"{W.shape} windows, newest slice == S"


def test_linear_latent_roundtrips(v):
    ...
    assert ok, f"round-trip nmse {rt:.1e}, torch vs numpy {d:.1e}"


def test_branched_linear_matches_podlse(v):
    ...
    assert ok, f"train {tr_closed:.4f}<={tr_learned:.4f}, test ratio {ratio:.2f}"


def test_branch_beats_the_linear_floor(v):
    ...
    assert e_nl < e_lin / 2, f"{e_lin:.4f} -> {e_nl:.4f}"


def test_warmup_is_enforced(v):
...
def test_torch_latent_wraps_an_autoencoder(v):
...
def test_forecaster_beats_persistence(v):
    ...
    assert e < pe, f"gru {e:.4f} < persistence {pe:.4f} at h={h}"
...
def test_case_flat_roundtrips_with_holes(v):
    ...
    assert ok, f"{Q.shape}, holes preserved, max |diff| {d:.1e}"


def test_build_case_reads_an_rds_shaped_run(v):
    """The full reader path: snapshots, transpose, v sign, pairing, drift, sync."""
    root = tempfile.mkdtemp()
    try:
        run, c = _write_fixture(root, n_t=120)
        # mask_tol=0 pins the strict rule on purpose: this test is about the
        # reader path -- (Ny, Nx) storage, the v sign flip, pair indices, drift,
        # sync -- and `toy` builds its reference Q with the body dropped. The
        # mask rule itself is tested in test_mask_rule.py, which is where a
        # change to the default belongs.
        case = we.build_case(run, root=root, mask_tol=0.0, verbose=v)
        Q = case.flat()
        ...
        assert ok, f"field {d_q:.1e}, sensors {d_s:.1e}, drift {case.drift_applied}"
    finally:
        shutil.rmtree(root, ignore_errors=True)


def test_force_sync_picks_the_right_samples(v):
    ...
    assert bool(ok), "decimate and block agree with the derived offset"


def test_drift_correction_removes_the_ramp(v):
    ...
    try:
        ...
        assert d < 1e-9, f"residual {d:.1e} after removing a known ramp"
    finally:
        shutil.rmtree(root, ignore_errors=True)


def test_study_script_runs_end_to_end(v):
    ...
    """
    if not _SLOW:
        pytest.skip("set VERIFY_SLOW=1 to run the end-to-end checks")
    root = tempfile.mkdtemp()
    ...
    try:
        ...
        cmd = [sys.executable,
               os.path.join(SCRIPTS, "sparse_sensor_study.py"),
        ...
        assert ok, f"rc={p.returncode}, {n_rows} rows in results.csv"
    finally:
        shutil.rmtree(root, ignore_errors=True)
        shutil.rmtree(out, ignore_errors=True)


def test_sweep_script_runs_and_resumes(v):
    ...
    try:
        ...
        env = {**os.environ, "RDS_ROOT": root, "MPLBACKEND": "Agg"}
        cmd = [sys.executable,
               os.path.join(SCRIPTS, "sparse_sensor_sweep.py"),
        ...
        assert ok, f"{n1} rows, {n2} after resume, video pack {pack}"
    finally:
        shutil.rmtree(root, ignore_errors=True)
        shutil.rmtree(out, ignore_errors=True)


def test_video_pack_renders(v):
    ...
    try:
        ...
        np.savez_compressed(
            ...
            meta=json.dumps({"dt": 0.004, "run": "toy",
                             "labels": {"pred_0": {"label": "toy", "nmse": 0.01,
                                                   "pinned": True}}}))
        script = os.path.join(SCRIPTS, "make_reconstruction_video.py")
        ...
        for extra in (["--format", "gif"],
            ...
            if r.returncode != 0:
                ...
                assert False, f"rc={r.returncode} on {extra}"
            made += [f for f in os.listdir(d) if f.endswith(".gif")]
        assert len(set(made)) >= 3, f"{len(set(made))} animations from one pack"
    finally:
