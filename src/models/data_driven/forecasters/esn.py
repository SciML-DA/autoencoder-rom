...
class ESN_model(EchoStateNetwork, Model):
    ...
    **kwargs
    ...
    def __init__(self, dt, **kwargs):
        ...
        values = np.atleast_1d(np.asarray(kwargs.pop("param_values", np.zeros(len(self.param_names))), dtype=float))
        ...
        if self.param_names:
            ...
            self.alpha_labels = dict(zip(self.param_names, param_labels or [f"${n}$" for n in self.param_names]))
        ...
        **kwargs
        ...
        if self.ensemble_cfg is not None:
            ...
            if "Wout" in self.est_alpha:
                self.est_alpha = [a for a in self.est_alpha if a != "Wout"] + [f"svd_{qi}" for qi in range(self.N_dim)]
    ...
    def Wout_Vh(self, Vh):
        ...
        """
        assert Vh.shape == (self.N_dim, self.N_dim), f"Expected shape ({self.N_dim}, {self.N_dim}), got {Vh.shape}"
        self._Wout_Vh = Vh
        ...
        **kwargs,
        ...
        if ensemble_psi0 is None:
            ...
            if isinstance(std_alpha, dict) and "Wout" in std_alpha:
                ...
                std_alpha.update({f"svd_{qi}": std_alpha.pop("Wout") for qi in range(self.N_dim)})
            ...
            if other:
                ...
                sub_std = {a: std_alpha[a] for a in other} if isinstance(std_alpha, dict) else std_alpha
                alpha.update(
                    zip(
                        other,
                        mean_vector_to_ensemble(
                            self.rng, np.array(means, dtype=float), sub_std, m, method=distribution_alpha
                        ),
                    )
                )

            ensemble_psi0 = (
                phi if not est_alpha else np.concatenate([phi, np.stack([alpha[name] for name in est_alpha])], axis=0)
            )

        out = super().init_ensemble(
            m=m,
            est_alpha=est_alpha,
            std_alpha=std_alpha,
            distribution_alpha=distribution_alpha,
            ensemble_psi0=ensemble_psi0,
            **kwargs,
        )
    ...
    def time_step(self, Nt=10, averaged=False):
        ...
        if self.param_names:
            ...
            self.input_parameters = np.array([[a[name] for a in alpha] for name in self.param_names])
    ...
    def plot_training_data(case, train_data, dt=None):
        ...
        for l, data_l in enumerate(train_data):
            ...
            for kk, ax in enumerate(axs_dim):
                ...
                ax.axvspan(0, case.t_train, facecolor="orange", alpha=0.3, zorder=-100, label="Train")
                ax.axvspan(
                    case.t_train, case.t_train + case.t_val, facecolor="red", alpha=0.3, zorder=-100, label="Validation"
                )
                ax.axvspan(
    ...
    def visualize_spatiotemporal_hist(
        self, y_hist=None, t=None, averaged=False, reference_y=1.0, reference_t: float = 1.0, **kwargs
    ):
    ...
    def plot_Wout(self):
        ...
        if not self.Wout_svd:
            ...
            fig, ax = plt.subplots()
            im = ax.matshow(self.Wout.T, cmap="PRGn", aspect=4.0, vmin=-np.max(self.Wout), vmax=np.max(self.Wout))
