...
three hundred. See the rank-ceiling note in ``tools/epod.py``.
...
CSV_FIELDS = [
    "tag", "runs", "test_run", "model", "latent", "branch", "r_field", "r_sensor",
    "n_delays", "ridge", "n_params", "nmse_train", "nmse_test", "nmse_latent",
...
def run_linear(Q, S, tr, te, args, Psi, q_mean, results, per_snap, preds,
    ...
    for L in args.delays:
        ...
        if args.ridge_cv:
            t0 = time.time()
            ridge, grid = ridge_cv(Q, Sd, tr, k=args.cv_folds, gap=args.gap,
                                   r_sensor=r_s, standardise_sensors=True)
            print(f"    ridge CV (L={L}): {ridge:g}  "
        ...
        for name, ctor in (("podlse", PODLSE), ("epod", ExtendedPOD)):
            t0 = time.time()
            if name == "podlse":
                m = ctor(r_field=args.r_field, r_sensor=r_s, ridge=ridge,
            ...
            label = f"{name} L={L}"
            results.append(dict(
                model=name, latent="pod", branch="closed-form",
                r_field=args.r_field if name == "podlse" else -1, r_sensor=r_s,
                n_delays=L, ridge=ridge, n_params=m.n_params,
                nmse_train=m.score(Q[:, tr], Sd[:, tr]),
                nmse_test=nmse(Q[:, te], pred), nmse_latent=lat,
...
def build_latents(Q, tr, args, case0, unflat):
    ...
    if "pod" in args.latents:
        t0 = time.time()
        Psi, Sigma, _, q_mean = pod(Q[:, tr], r=args.r_field, subtract_mean=True,
                                    method=args.pod_method, seed=args.seed)
    ...
    for lname, (lat, Psi, _, q_mean) in latents.items():
        ...
        for branch in args.branches:
            for L in args.delays:
                ...
                t0 = time.time()
                m = BranchedAE(
                    lat, branch=branch, n_delays=L, delay_stride=args.delay_stride,
                    hidden=tuple(args.hidden), gru_hidden=args.gru_hidden,
                    ...
                    device=args.device, seed=args.seed, verbose=args.verbose,
                ).fit(Q, S, tr)
                dt = time.time() - t0
                pred = m.predict(S, te)
                label = f"{lname}+{branch} L={L}"
                results.append(dict(
                    model="branched", latent=lname, branch=branch,
                    r_field=args.r_field, r_sensor=-1, n_delays=L,
                    ridge=float("nan"), n_params=m.n_params,
...
def run_forecast(Q, S, tr, te, args, latents, results):
    ...
    f = LatentForecaster(n_latent=lat.n_latent, n_delays=L, n_unroll=args.n_unroll,
                         hidden=args.gru_hidden, n_epochs=args.epochs,
                         device=args.device, seed=args.seed,
                         verbose=args.verbose).fit(Z_tr, np.arange(len(tr)))

    Z_te_true = lat.encode(Q[:, te])
    nowcast = BranchedAE(lat, branch="gru", n_delays=L, n_epochs=args.epochs,
                         gru_hidden=args.gru_hidden, device=args.device,
                         seed=args.seed).fit(Q, S, tr)
    ...
    for src, Z0 in sources:
        errs = []
        for h in horizons:
            e = [nmse(Q[:, te[t0 : t0 + h]], lat.decode(f.rollout(Z0[:, t0 - L : t0], h)), var)
                 for t0 in starts]
    ...
    for h in horizons:
        e = [nmse(Q[:, te[t0 : t0 + h]],
                  lat.decode(np.repeat(Z_te_true[:, t0 - 1 : t0], h, axis=1)), var)
             for t0 in starts]
        errs.append(float(np.mean(e)))
...
def _record_forecast(results, args, lname, branch, horizons, errs):
    for h, v in zip(horizons, errs):
        results.append(dict(
            model="forecast", latent=lname, branch=branch, r_field=args.r_field,
            r_sensor=-1, n_delays=h, ridge=float("nan"), n_params=0,
            nmse_train=float("nan"), nmse_test=v, nmse_latent=float("nan"),
            floor_test=float("nan"), fit_seconds=0.0,
...
def _say(r: dict) -> None:
    print(f"    {r['label']:28s} test {r['nmse_test']:.4f}  "
          f"train {r['nmse_train']:.4f}  floor {r['floor_test']:.4f}  "
          f"{r['n_params']:>9,d} par  {r['fit_seconds']:.0f}s", flush=True)
...
def write_csv(path, rows, args, tr, te, held):
    ...
    with open(path, "a", newline="") as fh:
        ...
        if new:
            w.writeheader()
        for r in rows:
            w.writerow({**r, "tag": args.tag, "runs": "|".join(args.runs or [args.run]),
                        "test_run": held, "seed": args.seed,
...
def main() -> int:
    ...
    m.add_argument("--branches", nargs="*", default=["linear", "mlp", "gru"],
    ...
    print(f"  best: {best['label']}  test NMSE {best['nmse_test']:.4f} "
          f"(floor {best['floor_test']:.4f}, mean-predictor 1.0)")
    if best.get("nmse_by_run"):
        print("\n  per run, each normalised by its own test block "
              "(comparable with a single-run job):")
    ...
    if not args.no_plots:
        rule("7. plots")
        obs = mode_observability(Psi.T @ (Q[:, tr] - q_mean),
                                 _sensor_coeffs(S, tr, args))
        rp.plot_spectrum(Sigma, os.path.join(out, "spectrum.png"),
                         floors={f"r={args.r_field}": args.r_field})
        rp.plot_observability(obs, os.path.join(out, "observability.png"))
        if "psi_ext" in _STASH:
            rp.plot_extended_modes(_STASH["psi_ext"], unflat,
                                   os.path.join(out, "extended_modes.png"),
                                   n_modes=args.n_ext_modes, mf=case0.mf)
        ...
        rp.plot_error_history(per_snap, os.path.join(out, "error_history.png"),
                              dt=args.stride / F_PIV_HZ)
        if curves:
            rp.plot_horizon(curves, os.path.join(out, "horizon.png"),
                            dt=args.stride / F_PIV_HZ)
...
def _sensor_coeffs(S, tr, args):
    Sd = delay_embed(S, max(args.delays), args.delay_stride,
                     args.delay_ahead)[:, tr]
...
def _animate(best, Q, te, preds, args, unflat, case0, out):
    ...
    if pred is None:
        ...
        return
    n = pred.shape[1]
    rp.animate_reconstruction(unflat(Q[:, te[:n]]), unflat(pred.astype(np.float64)),
                              os.path.join(out, "reconstruction.gif"),
                              mf=case0.mf, n_frames=n,
